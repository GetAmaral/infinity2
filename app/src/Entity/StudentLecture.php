...
use App\Entity\Generated\StudentLectureGenerated;
...
class StudentLecture extends StudentLectureGenerated
{
    ...
    // === COMPLETION PERCENTAGE METHODS ===

    // Removed override - using parent's string-based completion percentage
    ...
    {
        return $this->getCompletionPercentage() >= 25.0;
    }
    ...
    {
        return $this->getCompletionPercentage() >= 50.0;
    }
    ...
    {
        return $this->getCompletionPercentage() >= 75.0;
    }
    ...
    public function calculateCompletion(): void
    {
        ...
        if ($this->getWatchedSeconds() > 0 && $this->firstWatchedAt === null) {
            $this->firstWatchedAt = new \DateTimeImmutable();
        }

        $lectureLength = $this->getLecture()->getDurationSeconds();

        error_log(sprintf('[PreUpdate] Calculating completion: watchedSeconds=%d, lectureLength=%d',
            $this->getWatchedSeconds(),
            $lectureLength
        ));

        if ($lectureLength > 0) {
            // Lecture with video - calculate based on watched time
            $percentage = ($this->getWatchedSeconds() / $lectureLength) * 100;
            $this->setCompletionPercentage(min($percentage, 100.0)); // Max 100%
            error_log(sprintf('[PreUpdate] Calculated percentage: %.2f%%', $this->getCompletionPercentage()));
        } else {
            // Videoless lecture - mark as 100% if any watched seconds recorded, 0% otherwise
            if ($this->getWatchedSeconds() > 0) {
                $this->setCompletionPercentage(100.0);
                error_log('[PreUpdate] Videoless lecture marked as complete (100%)');
            } else {
                $this->setCompletionPercentage(0.0);
                error_log('[PreUpdate] Videoless lecture not started (0%)');
            }
        }

        // Auto-mark as completed if >= MIN_COMPLETION
        if ($this->getCompletionPercentage() >= self::MIN_COMPLETION) {
            if (!$this->isCompleted()) {
    ...
    {
        if ($this->getStudentCourse() !== null) {
            $this->getStudentCourse()->recalculateProgress();
        }
    }
