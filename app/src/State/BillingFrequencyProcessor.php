...
/**
 * Billing Frequency State Processor
 *
 ...
 */
class BillingFrequencyProcessor implements ProcessorInterface
{
    ...
    /**
     * @param BillingFrequencyInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): BillingFrequency
    {
        if (!$data instanceof BillingFrequencyInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(BillingFrequency::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('BillingFrequency not found');
            }
        }

        if (!$entity) {
            $entity = new BillingFrequency();
        }
        ...
        if (!$isPatch || array_key_exists('intervalType', $requestData)) {
            $entity->setIntervalType($data->intervalType);
        }
        // intervalCount
        if (!$isPatch || array_key_exists('intervalCount', $requestData)) {
            $entity->setIntervalCount($data->intervalCount);
        }
        // daysInCycle
        if (!$isPatch || array_key_exists('daysInCycle', $requestData)) {
            $entity->setDaysInCycle($data->daysInCycle);
        }
        // discountPercentage
