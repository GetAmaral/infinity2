 ...
 */
class StepActionProcessor implements ProcessorInterface
{
    ...
    /**
     * @param StepActionInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): StepAction
    {
        if (!$data instanceof StepActionInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(StepAction::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('StepAction not found');
            }
        }

        if (!$entity) {
            $entity = new StepAction();
        }
        ...
        // fewShot
        if (!$isPatch || array_key_exists('fewShot', $requestData)) {
            $entity->setFewShot($data->fewShot);
        }
