 ...
 */
class ProductLineProcessor implements ProcessorInterface
{
    ...
    /**
     * @param ProductLineInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): ProductLine
    {
        if (!$data instanceof ProductLineInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(ProductLine::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('ProductLine not found');
            }
        }
