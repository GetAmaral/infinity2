 ...
 */
class DealTypeProcessor implements ProcessorInterface
{
    ...
    /**
     * @param DealTypeInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): DealType
    {
        if (!$data instanceof DealTypeInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(DealType::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('DealType not found');
            }
        }
        ...
        if (!$isPatch || array_key_exists('expectedDuration', $requestData)) {
            $entity->setExpectedDuration($data->expectedDuration);
        }
        // winProbability
        if (!$isPatch || array_key_exists('winProbability', $requestData)) {
            $entity->setWinProbability($data->winProbability);
        }
