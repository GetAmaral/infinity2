...
use App\Dto\ProfileTemplateInputDto;
...
/**
 * ProfileTemplate State Processor
 *
 ...
 */
class ProfileTemplateProcessor implements ProcessorInterface
{
    ...
    /**
     * @param ProfileTemplateInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): ProfileTemplate
    {
        if (!$data instanceof ProfileTemplateInputDto) {
        ...
        if (isset($uriVariables['id'])) {
            $entity = $this->entityManager->getRepository(ProfileTemplate::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('ProfileTemplate not found');
            }
        }

        if (!$entity) {
            $entity = new ProfileTemplate();
        }
