 ...
 */
class StepInputProcessor implements ProcessorInterface
{
    ...
    /**
     * @param StepInputInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): StepInput
    {
        if (!$data instanceof StepInputInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(StepInput::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('StepInput not found');
            }
        }

        if (!$entity) {
            $entity = new StepInput();
        }
