 ...
 */
class LeadSourceProcessor implements ProcessorInterface
{
    ...
    /**
     * @param LeadSourceInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): LeadSource
    {
        if (!$data instanceof LeadSourceInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(LeadSource::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('LeadSource not found');
            }
        }

        if (!$entity) {
            $entity = new LeadSource();
        }
