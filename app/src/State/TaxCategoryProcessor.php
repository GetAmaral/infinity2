 ...
 */
class TaxCategoryProcessor implements ProcessorInterface
{
    ...
    /**
     * @param TaxCategoryInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): TaxCategory
    {
        if (!$data instanceof TaxCategoryInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(TaxCategory::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('TaxCategory not found');
            }
        }
        ...
        // taxRate
        if (!$isPatch || array_key_exists('taxRate', $requestData)) {
