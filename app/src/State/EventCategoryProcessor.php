...
use App\Dto\EventCategoryInputDto;
 ...
 */
class EventCategoryProcessor implements ProcessorInterface
{
    ...
    /**
     * @param EventCategoryInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): EventCategory
    {
        if (!$data instanceof EventCategoryInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(EventCategory::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('EventCategory not found');
            }
        }
        ...
        // allowMultiple
        if (!$isPatch || array_key_exists('allowMultiple', $requestData)) {
            $entity->setAllowMultiple($data->allowMultiple);
        }
        // active
