 ...
 */
class FlagProcessor implements ProcessorInterface
{
    ...
    /**
     * @param FlagInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): Flag
    {
        if (!$data instanceof FlagInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(Flag::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('Flag not found');
            }
        }
        ...
        // entityType
        if (!$isPatch || array_key_exists('entityType', $requestData)) {
        ...
        // dueDate
        if (!$isPatch || array_key_exists('dueDate', $requestData)) {
