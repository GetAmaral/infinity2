...
use App\Dto\TalkTypeTemplateInputDto;
...
/**
 * TalkTypeTemplate State Processor
 *
 ...
 */
class TalkTypeTemplateProcessor implements ProcessorInterface
{
    ...
    /**
     * @param TalkTypeTemplateInputDto $data
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): TalkTypeTemplate
    {
        if (!$data instanceof TalkTypeTemplateInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(TalkTypeTemplate::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('TalkTypeTemplate not found');
            }
        }

        if (!$entity) {
            $entity = new TalkTypeTemplate();
        }
        ...
        // iconUrl
        if (!$isPatch || array_key_exists('iconUrl', $requestData)) {
