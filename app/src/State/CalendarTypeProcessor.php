 ...
 */
class CalendarTypeProcessor implements ProcessorInterface
{
     ...
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): CalendarType
    {
        if (!$data instanceof CalendarTypeInputDto) {
        ...
        if ($isUpdate) {
            $entity = $this->entityManager->getRepository(CalendarType::class)->find($uriVariables['id']);
            if (!$entity) {
                throw new BadRequestHttpException('CalendarType not found');
            }
        }

        if (!$entity) {
            $entity = new CalendarType();
        }
