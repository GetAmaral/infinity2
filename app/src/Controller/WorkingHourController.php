<?php

declare(strict_types=1);

namespace App\Controller;

use App\Controller\Generated\WorkingHourControllerGenerated;
use Symfony\Component\Routing\Attribute\Route;

/**
 * WorkingHour Controller
 *
 * This class extends the generated base and is SAFE TO EDIT.
 * Add custom actions and override methods here.
 *
 * @generated once by Luminai Code Generator
 */
#[Route('/working-hour')]
class WorkingHourController extends WorkingHourControllerGenerated
{
    // Add custom actions here

    // Example:
    // #[Route('/custom-action', name: 'working_hour_custom')]
    // public function customAction(): Response
    // {
    //     return $this->render('workinghour/custom.html.twig');
    // }
}
