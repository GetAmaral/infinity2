<?php

declare(strict_types=1);

namespace App\Controller;

use App\Controller\Generated\TaskTemplateControllerGenerated;
use Symfony\Component\Routing\Attribute\Route;

/**
 * TaskTemplate Controller
 *
 * This class extends the generated base and is SAFE TO EDIT.
 * Add custom actions and override methods here.
 *
 * @generated once by Luminai Code Generator
 */
#[Route('/task-template')]
class TaskTemplateController extends TaskTemplateControllerGenerated
{
    // Add custom actions here

    // Example:
    // #[Route('/custom-action', name: 'task_template_custom')]
    // public function customAction(): Response
    // {
    //     return $this->render('tasktemplate/custom.html.twig');
    // }
}
