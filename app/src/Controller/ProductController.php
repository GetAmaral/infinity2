<?php

declare(strict_types=1);

namespace App\Controller;

use App\Controller\Generated\ProductControllerGenerated;
use App\Entity\Product;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Attribute\Route;

/**
 * Product Controller
 *
 * This controller handles all product operations.
 * Custom business logic can be added by overriding lifecycle hooks.
 *
 * @see ProductControllerGenerated for available lifecycle hooks
 */
#[Route('/product')]
final class ProductController extends ProductControllerGenerated
{
    /**
     * List all products
     */
    #[Route('', name: 'product_index', methods: ['GET'])]
    public function index(): Response
    {
        return $this->indexAction();
    }

    /**
     * API endpoint for searching products
     * Used by list page for dynamic data loading
     */
    #[Route('/api/search', name: 'product_api_search', methods: ['GET'])]
    public function apiSearch(Request $request): Response
    {
        return $this->apiSearchAction($request);
    }

    /**
     * Create new product
     * Handles both GET (show form) and POST (process form)
     */
    #[Route('/new', name: 'product_new', methods: ['GET', 'POST'])]
    public function new(Request $request): Response
    {
        if ($request->isMethod('POST')) {
            return $this->createAction($request);
        }

        return $this->newFormAction($request);
    }

    /**
     * Edit existing product
     * Handles both GET (show form) and POST (process form)
     */
    #[Route('/{id}/edit', name: 'product_edit', methods: ['GET', 'POST'])]
    public function edit(Product $product, Request $request): Response
    {
        if ($request->isMethod('POST')) {
            return $this->updateAction($product, $request);
        }

        return $this->editFormAction($product, $request);
    }

    /**
     * Delete product
     */
    #[Route('/{id}', name: 'product_delete', methods: ['POST'])]
    public function delete(Product $product, Request $request): Response
    {
        return $this->deleteAction($product, $request);
    }

    /**
     * Show product details
     */
    #[Route('/{id}', name: 'product_show', methods: ['GET'])]
    public function show(Product $product): Response
    {
        return $this->showAction($product);
    }

    // ====================================
    // CUSTOM METHODS & LIFECYCLE HOOKS
    // ====================================

    // Example: Override lifecycle hooks for custom logic
    //
    // protected function afterCreate(Product $product): void
    // {
    //     // Send notification email
    //     // $this->emailService->sendCreationNotification($product);
    //
    //     // Trigger domain event
    //     // $this->eventDispatcher->dispatch(new ProductCreatedEvent($product));
    // }
    //
    // protected function beforeDelete(Product $product): void
    // {
    //     // Check for dependencies
    //     // if ($product->hasRelatedRecords()) {
    //     //     throw new \RuntimeException('Cannot delete product with related records');
    //     // }
    // }

    // Add your custom controller methods here
}
